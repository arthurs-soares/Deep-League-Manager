...
const { sendLogMessage, manageGuildForumPost } = require('../handlers'); // NOVO: manageGuildForumPost importado
    ...
    async execute(interaction, client, globalConfig) { 
        ...
        try {
            ...
            if (confirmation.customId === `confirm_delete_${guild.name.toLowerCase().replace(/\s+/g, '-')}`) {
                ...
                // Remove duplicatas se houver (um membro pode ser líder E estar no roster, por exemplo)
                const uniqueMemberIds = [...new Set(allMembersIds)];

                // Cooldown de 3 dias (recuperado de globalConfig ou padrão)
                const COOLDOWN_DAYS = 3; 
                const COOLDOWN_MILLISECONDS_LOCAL = COOLDOWN_DAYS * 24 * 60 * 60 * 1000;

                // Atualiza a lista `recentlyLeftUsers` na globalConfig
                ...
                // NOVO: Deletar o post do fórum da guilda
